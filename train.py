...
if __name__ == '__main__':
    ...
    parser.add_argument('--pre-trained', type=bool, default=False, help='whether load pre-trained parameters')
    parser.add_argument('--checkpoint', type=str, default='./checkpoint', help='path to pre-trained parameters')
    parser.add_argument('--train-root', type=str, default='G:/dataset/RGB-D/train_dataset/train_2985/', help='path to the train dataset')
    #超参数设置
    parser.add_argument('--epochs', type=int, default=200)
    parser.add_argument('--batch-size', type=int, default=8, help='total batch size for all GPUs')
    parser.add_argument('--workers', type=int, default=4, help='maximum number of dataloader workers')
    parser.add_argument('--lr', type=float, default=0.0005, help='learning rate')
    parser.add_argument('--weight_decay', type=float, default=0.0001, help='decay rate of learning rate')
    ...
    if args.pre_trained is True:
        ...
        # 加载之前训练好的权重
        pretrained_dict = torch.load(os.path.join('./checkpoint/RGBD-SOD_iter_' + str(args.ckpt) + '.pth'))
        # 将pretained_dict里不属于model_dict的键剔除掉
        #pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        # 只加载主干网络
        RGB_net = 'baseline_net'
        depth_net = 'depth_net'
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if RGB_net in k or depth_net in k}
        # 更新现有的model_dict
