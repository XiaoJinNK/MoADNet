...
class IBCMF(nn.Module):
    ...
    def __init__(self, channel_RGB, channel_Depth, channel_in, channel_out):
        ...
        #channel expansion
        self.conv_rgb = nn.Sequential(nn.Conv2d(channel_RGB, channel_in, 1, 1, 0), nn.BatchNorm2d(channel_in), nn.ReLU(inplace=True))
        self.conv_depth = nn.Sequential(nn.Conv2d(channel_Depth, channel_in, 1, 1, 0), nn.BatchNorm2d(channel_in), nn.ReLU(inplace=True))

        #feature fusion
        self.SA_module_rgb = sa_layer(channel_in, groups=channel_in // 2)
        self.SA_module_depth = sa_layer(channel_in, groups=channel_in // 2)

        self.conv_3 = nn.Sequential(nn.Conv2d(channel_in*2, channel_in, 1, 1, 0), nn.BatchNorm2d(channel_in), nn.ReLU(inplace=True))
        self.conv_4 = nn.Sequential(nn.Conv2d(channel_in, 2, 3, 1, 1), nn.Sigmoid())

        #channel projection
        self.conv_out = nn.Sequential(nn.Conv2d(channel_in, channel_in // 2, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(channel_in // 2, channel_in // 4, 3, 1, 1), nn.ReLU(inplace=True),
                                      nn.Conv2d(channel_in // 4, channel_out, 3, 1, 1), nn.ReLU(inplace=True))
