...
def test(test_root, model_fusion, MapRoot, cuda):
    ...
    for id, (data, depth, img_name, img_size) in enumerate(test_loader):
        ...
        #保存所有阶段图片
        # F2_out, F3_out, F4_out, F5_out, F_last = model_fusion(inputs, depth)
        #
        ...
        # stage5_pred = torch.sigmoid(F5_out)[0, 0].detach().cpu().numpy()
        # stage_last_pred = torch.sigmoid(F_last)[0, 0].detach().cpu().numpy()
        #
        # plt.imsave(os.path.join(MapRoot, img_name[0] + '_stage2.png'), stage2_pred, cmap='gray')
        # plt.imsave(os.path.join(MapRoot, img_name[0] + '_stage3.png'), stage3_pred, cmap='gray')
        # plt.imsave(os.path.join(MapRoot, img_name[0] + '_stage4.png'), stage4_pred, cmap='gray')
        # plt.imsave(os.path.join(MapRoot, img_name[0] + '_stage5.png'), stage5_pred, cmap='gray')
        # plt.imsave(os.path.join(MapRoot, img_name[0] + '_stage_last.png'), stage_last_pred, cmap='gray')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', type=str, default='./checkpoint/RGBD-SOD_iter_0.pth', help='num of checkpoints')
